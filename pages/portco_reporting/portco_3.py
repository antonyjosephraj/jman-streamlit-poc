...
def main():
    ...
    if 'max_date_pf3' not in ss:
        max_date = investments_details['Exit Date'].max()
        ss.max_date_pf3 = datetime.strptime(max_date, '%Y-%m-%d').date()
    ...
    ss.investments_data_pf3['Exit Date'] = pd.to_datetime(ss.investments_data_pf3['Exit Date']).dt.date
    max_date = ss.investments_data_pf3['Exit Date'].max()
    max_date_str_pf3 = max_date.strftime('%Y-%m-%d')
    max_date = datetime.strptime(max_date_str_pf3, '%Y-%m-%d').date()
    ss.max_date_pf3 = max_date
    ...
    if 'assumptions_data_pf3' not in ss:
        ss.assumptions_data_pf3 = pd.DataFrame(assumptions)
    ...
    def fetching_assumptions_data():
        ...
        max_date = ss.investments_data_pf3['Exit Date'].max()
        sample_data = ss.assumptions_data_pf3
        sample_data['Date'] = pd.to_datetime(sample_data['Date'], format='%Y-%m-%d').dt.strftime('%Y-%m-%d')
        ss.max_date_pf3 = datetime.strptime(max_date.strftime('%Y-%m-%d'), '%Y-%m-%d').date()

        date_range = pd.date_range(start=min_date, end=max_date, freq='ME')
        ss.assumptions_data_pf3 = pd.DataFrame(date_range, columns=['Date'])
        ss.assumptions_data_pf3['Date'] = pd.to_datetime(ss.assumptions_data_pf3['Date'], format='%Y-%m-%d').dt.strftime('%Y-%m-%d')
        ss.assumptions_data_pf3[["Low Case" ,"Base Case" ,"High Case"]] = 0
        ss.assumptions_data_pf3[["Comments"]] = None

        merged_df = pd.merge(ss.assumptions_data_pf3, sample_data, on=['Date'], suffixes=('_df1', '_df2'), how='left')
        ...
        ss.assumptions_data_pf3 = merged_df[['Date', "Low Case" ,"Base Case" ,"High Case" ,"Comments"]].drop_duplicates()
        ...
        investment_update = ss.assumptions_data_pf3
    ...
    with st.container(border=True):
        ...
        with col1:
            ...
            def handle_date_change(date):
                ...
                print('JJJJJJJJJJJ', date)
                ss.min_date_pf3 = date
        ...
        if 'netdebt_and_cashflow_df_pf3' not in ss:
            ss.netdebt_and_cashflow_df_pf3 = netdebt_and_cashflow_df_pf3
        ...
        with column2:
            ...
            with st.container(height=400, border=True):
                ...
                with tab1:
                    ...
                    editda_multiple_df_pf3_styled_df = ss.editda_multiple_df_pf3
                    st.markdown(editda_multiple_df_pf3_styled_df.style.hide(axis="index").set_table_attributes('style="margin: 0 auto;"').to_html(), unsafe_allow_html=True)
                ...
                with tab2:
                    ...
                    netdebt_and_cashflow_edited_df_pf3 = de(ss.netdebt_and_cashflow_df_pf3, width=550, height=110, hide_index=True, disabled=["Calc"])

                concatenated_df = pd.concat([ss.editda_multiple_df_pf3, ss.netdebt_and_cashflow_df_pf3], ignore_index=True)
                ...
                with tab3:
                    ...
                    equity_df_pf3_styled_df = ss.equity_df_pf3
                    st.markdown(equity_df_pf3_styled_df.style.hide(axis="index").set_table_attributes('style="margin: 0 auto;"').to_html(), unsafe_allow_html=True)
                ...
                with tab4:
                    ...
                    ownership_edited_df_pf3 = de(ss.ownership_df_pf3, width=550, height=70, hide_index=True, disabled=["Calc"])
                    
                concatenated_df_v2 = pd.concat([ss.equity_df_pf3, ss.ownership_df_pf3], ignore_index=True)
                ...
                ss.value_and_investment_df_pf3 = value_and_investment_df
                ...
                with tab5:
                    ...
                    value_and_investment_df_pf3_styled_df = ss.value_and_investment_df_pf3
                    st.markdown(value_and_investment_df_pf3_styled_df.style.hide(axis="index").set_table_attributes('style="margin: 0 auto;"').to_html(), unsafe_allow_html=True)
                ...
                with tab6:
                    ...
                    money_multiple_df_pf3_styled_df = ss.money_multiple_df_pf3
                    st.markdown(money_multiple_df_pf3_styled_df.style.hide(axis="index").set_table_attributes('style="margin: 0 auto;"').to_html(), unsafe_allow_html=True)
        ...
        revenue_return_pf3_styled_df = style_dataframe(ss.revenue_return_pf3)
    ...
    with st.container(border=True):
        ...
        with st.form("my_form"):
            ...
            fig = go.Figure(go.Waterfall(
                ...
                # text = ['Low', 'Base', 'High'],
                y = waterfall_data_flow_df_pf3['Values'],
